...
import re
from util import format_checkers
...
class Client:
    ...
    def dni(self, dni):
        ...
        Asigna el valor del dni si el formato es
        correcto y si no lo es, lanza una excepción

        :param dni: el dni

        :type dni: string
        """
        if format_checkers.valid_dni_format(dni):
            self.__dni = dni
        else:
